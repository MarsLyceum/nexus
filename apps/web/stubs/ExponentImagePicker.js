...
export default {
    async launchImageLibraryAsync({
        ...
        // SSR guard: if not running in the browser, immediately return canceled.
        if (!isBrowser) {
...
function openFileBrowserAsync({
    ...
    // Return a promise that resolves when a file is selected or canceled
    return new Promise((resolve) => {
        input.addEventListener('change', async () => {
            if (input.files?.length) {
                const files = allowsMultipleSelection
                    ? input.files
                    : [input.files[0]];
...
function createMediaTypeFormat(mediaTypes) {
    const filteredMediaTypes = mediaTypes.filter(
        (mediaType) => mediaType !== 'livePhotos'
    );
